...
// ─── Global event so any component (e.g. Navbar) can open the cart ────────────
export function openCartDrawer() {
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("starry:open-cart"));
  }
}
...
export function Cart({ whatsapp, primaryColor = "#0f766e", vendorName }: CartProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("starry:add-to-cart", addHandler);
    window.addEventListener("starry:open-cart", openHandler);
    return () => {
      window.removeEventListener("starry:add-to-cart", addHandler);
      window.removeEventListener("starry:open-cart", openHandler);
    };
  }, []);
...
// Reads live count from localStorage and opens the cart drawer via event.
export function NavbarCartButton({ primaryColor = "#0f766e" }: { primaryColor?: string }) {
  ...
  useEffect(() => {
    ...
    refresh();
    window.addEventListener("starry:add-to-cart", refresh);
    window.addEventListener("starry:cart-updated", refresh);
    return () => {
      window.removeEventListener("starry:add-to-cart", refresh);
      window.removeEventListener("starry:cart-updated", refresh);
    };
  }, []);

  // SSR placeholder (no count badge)
  if (!mounted) {
    return (
      <button
        aria-label="Panier"
        className="relative p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
      >
  ...
  return (
    ...
    >
      ...
      {count > 0 && (
        <span
          className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full text-white text-[10px] font-bold flex items-center justify-center shadow-md"
          style={{ backgroundColor: primaryColor }}
        >
          {count > 9 ? "9+" : count}
        </span>
